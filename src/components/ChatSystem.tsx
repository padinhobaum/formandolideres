...
export default function ChatSystem() {
  ...
  const startPrivateChat = async (targetUserId: string) => {
    ...
    { conversation_id: (conv as any).id, user_id: targetUserId }] as
    any);
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-r from-primary to-accent shadow-lg hover:opacity-90 transition-opacity flex items-center justify-center my-[60px]">
    ...
    <div className="fixed inset-0 z-50 bg-background flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3">
          ...
          <label key={u.user_id} className="flex items-center gap-3 p-2 cursor-pointer hover:bg-secondary/50">
              ...
              if (e.target.checked) setSelectedUsers([...selectedUsers, u.user_id]);else
