...
export default function ChatSystem() {
  ...
  const fetchAllUsers = async () => {
    ...
    rolesRes.data?.forEach((r: any) => adminIds.add(r.user_id));
    setAdminUserIds(adminIds);

    if (profilesRes.data) {
      const allProfiles = profilesRes.data as UserProfile[];
      const map: Record<string, UserProfile> = {};
      allProfiles.forEach((u) => { map[u.user_id] = u; });
      setProfilesMap(map);

      // Filter based on permissions:
      // Admins can message anyone; Leaders can only message admins
      if (isAdmin) {
        setAllUsers(allProfiles.filter((u) => u.user_id !== user?.id));
      } else {
        setAllUsers(allProfiles.filter((u) => u.user_id !== user?.id && adminIds.has(u.user_id)));
      }
    }
  };
  ...
  const startPrivateChat = async (targetUserId: string) => {
    ...
    // Permission check: leaders can only message admins
    if (!isAdmin && !adminUserIds.has(targetUserId)) {
      toast.error("Líderes só podem enviar mensagens para Administradores.");
      return;
    }
  ...
  const createGroup = async () => {
    ...
    // Leaders can only create groups with admins
    if (!isAdmin) {
      const hasNonAdmin = selectedUsers.some((uid) => !adminUserIds.has(uid));
      if (hasNonAdmin) {
        toast.error("Líderes só podem criar grupos com Administradores.");
        return;
      }
    }
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-r from-primary to-accent shadow-lg hover:opacity-90 transition-opacity flex items-center justify-center mb-[60px] md:mb-0"
      >
      ...
      {!activeConv && !showNewChat && !showNewGroup ? (
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-3">
            ...
            {!isAdmin && (
              <p className="text-xs text-muted-foreground mb-2 bg-secondary/50 p-2 rounded">
                Como líder, você pode enviar mensagens apenas para Administradores.
              </p>
            )}
              ...
              {filteredUsers.map((u) => (
                <button key={u.user_id} onClick={() => startPrivateChat(u.user_id)} className="w-full p-3 flex items-center gap-3 hover:bg-secondary/50 transition-colors text-left">
                  ...
                  {adminUserIds.has(u.user_id) && (
                    <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded ml-auto">Admin</span>
                  )}
                </button>
              ))}
        ...
        <div className="flex-1 overflow-y-auto p-3">
          ...
          {!isAdmin && (
            <p className="text-xs text-muted-foreground mb-2 bg-secondary/50 p-2 rounded">
              Como líder, você só pode adicionar Administradores ao grupo.
            </p>
          )}
              ...
              return (
                <div key={m.id} className={`flex gap-2 ${isMe ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`max-w-[75%] rounded-2xl px-3 py-2 text-sm ${isMe ? "bg-primary text-primary-foreground rounded-br-sm" : "bg-card border rounded-bl-sm"}`}>
                    {!isMe && <p className="text-[10px] font-medium mb-0.5 opacity-70">{m.sender_name}</p>}
